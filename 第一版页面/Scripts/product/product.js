	...
	*/
    ...
    function category() {
        ...
        $.each(categorydata, function (index, value) {			
            htmlcat += '<span class="tgr" data-idx="'+(index+1)+'" onclick="searchbag(this,2)" data-levle="1" data-categoryid="' + value.categoryid + '" id="lblcategory_' + value.categoryid + '">' + value.categoryname + '</span>';
        });
    ...
    search = function (p) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data.r == -1) {
                    $("#divcontent").html("<div style='text-align:center;margin-top:100px;'><h3>暂无符合条件的数据</h3></div>");
	...
	searchcat=function(obj){
		...
		var $obj = $(obj); 
        var categoryid = $obj.data("categoryid");
		$obj.addClass("checka").siblings().removeClass("checka");
